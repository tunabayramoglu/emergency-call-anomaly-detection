...
def evaluate_wer(model, dl, ds):
    model.eval()
    # NOTE: make sure the backbone is in eval mode too (we already left it that way)
    hyps, refs = [], []

    # the new DataLoader returns raw audio together with the true lengths
    for x_audio, y, x_audio_lens, ylen, idxs in dl:
        x_audio = x_audio.to(DEVICE, non_blocking=True)

        # step 1: run the audio through mHuBERT on the fly, as in training
        with torch.autocast(device_type="cuda", dtype=torch.float16):
            ...
            # valid output lengths
            xlen = backbone._get_feat_extract_output_lengths(x_audio_lens.to(DEVICE))

        # step 2: feed the layers to the head
        logits = model(hs.float())
        pred = logits.argmax(-1).cpu().numpy()

        for b, i in enumerate(idxs):
            # decode the prediction and align it with the reference text
            hyps.append(greedy_decode(pred[b, :xlen[b]].tolist()))
            # ds.ds[i]["text"] pulls the original text from the Dataset object
